...
// HandleValidationError returns a consistent error response using utils.ErrResponse
func HandleValidationError(ctx *gin.Context, err error) {
	if validationError, ok := err.(validator.ValidationErrors); ok {
		...
		utils.ErrorResponse(ctx, 400, "Validation error", errors)
		return
	}
	utils.ErrorResponse(ctx, 400, "Yêu cầu không hợp lệ", err.Error())
}
